...
class VGGLoss(nn.Module):
    ...
    def forward(self, src, tgt):
        src_feats = self.get_features(src)
        tgt_feats = self.get_features(tgt)
        loss = 0
        for i in range(len(src_feats)):
            loss += F.mse_loss(src_feats[i], tgt_feats[i], reduction=self.reduction)
        return loss
    

class VGGLossObjaverse(nn.Module):
    def __init__(self, model='vgg19', layer=8, shift=0, reduction='none'):
        super().__init__()
        self.shift = shift
        self.reduction = reduction
        self.normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
    ...
    def forward(self, src, tgt):
        src_feats = self.get_features(src)  # List of feature maps for source
        tgt_feats = self.get_features(tgt)  # List of feature maps for target

        # Initialize loss vector to store individual losses per sample in the batch
        batch_size = src.shape[0]
        loss = torch.zeros(batch_size, device=src.device)

        # Compute MSE loss for each feature layer, batch-wise
        for i in range(len(src_feats)):
            # MSE loss with reduction='none' returns (batch_size, C, H, W)
            mse_loss = F.mse_loss(src_feats[i], tgt_feats[i], reduction='none')
            # Take mean over C, H, W to get batch-wise loss
            mse_loss = mse_loss.mean(dim=[1, 2, 3])  # Shape: (batch_size,)
            loss += mse_loss

        return loss  # Return loss as a vector (batch_size,)
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, feat_src,  feat_tgt, prob_src, prob_tgt):
        feat_src = F.normalize(feat_src, dim=2, p=2)
        feat_tgt = F.normalize(feat_tgt, dim=2, p=2)

        sim = torch.matmul(feat_src, feat_tgt.permute(0, 2, 1)) / self.temperture
        weight = prob_src * prob_tgt
        weight = weight / weight.sum(1, keepdim=True)
        log_sim = F.log_softmax(sim, dim=2)
        loss = - (torch.diagonal(log_sim, dim1=1, dim2=2) * weight).sum(1)
        return loss.mean()