...
class Co3dDataset(Dataset):
    def __init__(
        ...
        eval_time=False,
        data_reduction_ratio=1.0,  # 新增参数，表示减少后的数据比例
    ):
        """
        Args:
        ...
        self.data_reduction_ratio = data_reduction_ratio  # 保存减少数据的比例
        ...
        self.sequence_list = list(self.rotations.keys())     #len sequence_list = len(dataset)  len = 22375
        self.split = split
        self.sequence_list = self.sequence_list[:int(len(self.sequence_list) * self.data_reduction_ratio)]     
    ...
    def get_data(self, index=None, sequence_name=None, ids=(0, 1), no_images=False):
        ...
        images_transformed, masks_transformed, imgs_size = [], [], []
        for i, (anno, image) in enumerate(zip(annos, images)):
            ...
            if self.cfg["DATA"]["OBJ_SIZE"] is None:
                ...
                bbox_jitter = self._jitter_bbox(bbox)      #bbox 长 和 宽相等  zhi
